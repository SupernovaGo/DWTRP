  ...
  return (
    <Card className="border-border/60 bg-background/40">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-violet-700 dark:text-violet-200">{t('🔄 世界 / 角色更新')}</CardTitle>
      </CardHeader>
      <CardContent className="grid grid-cols-2 gap-4">
        <TimeListField label={t("更新时间")} value={toTimeList(update.time)} onChange={(v) => patchUpdate({ time: fromTimeList(v) })} />
        <NumberField label={t("角色批次大小")} value={update.batch_size ?? 10} min={1} max={50} onChange={(v) => patchUpdate({ batch_size: v })} />
        <label className="flex items-center justify-between gap-2 text-sm">
          ...
          <Select value={update.world_first ? 'world' : 'character'} onValueChange={(v) => patchUpdate({ world_first: v === 'world' })}>
            <SelectTrigger className="h-9 w-40" size="sm">
              <SelectValue placeholder={t("顺序")} />
            </SelectTrigger>
            <SelectContent align="end">
              <SelectItem value="world">{t('世界在前')}</SelectItem>
              <SelectItem value="character">{t('角色在前')}</SelectItem>
            </SelectContent>
          </Select>
        </label>
        <label className="flex items-center justify-between gap-2 text-sm">
