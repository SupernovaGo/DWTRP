...
export default function IdentityTab() {
  ...
  const save = async () => {
    try {
      ...
      toast.push('身份已保存（仅影响当前会话）', '', 'success')
    } catch (e) {
    ...
    <div className="flex h-full flex-col overflow-hidden p-4">
      <Card className="flex min-h-0 flex-1 flex-col border-border/60 bg-background/40">
        <CardHeader className="shrink-0 pb-2">
          <CardTitle className="text-base font-semibold text-violet-700 dark:text-violet-200">🧑 当前会话身份</CardTitle>
        </CardHeader>
        <CardContent className="min-h-0 flex-1 space-y-3 overflow-y-auto">
          <label className="block text-sm"><span className="text-muted-foreground">名称</span>
            <Input className="mt-1.5" value={identity.name ?? ''} onChange={(e) => setIdentity({ ...identity, name: e.target.value })} />
          </label>
          <label className="block text-sm"><span className="text-muted-foreground">身份 / 角色</span>
            <Input className="mt-1.5" value={identity.role ?? ''} onChange={(e) => setIdentity({ ...identity, role: e.target.value })} />
          </label>
          <label className="block text-sm"><span className="text-muted-foreground">描述</span>
        ...
        <div className="shrink-0 space-y-2 border-t border-border/60 p-3">
          ...
          <p className="text-sm text-muted-foreground">改动只影响当前会话，不影响资源库里的身份卡模板。</p>
        </div>
      </Card>
    </div>
  )
