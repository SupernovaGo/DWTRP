...
export default function EnvSetupCard() {
    ...
    pollRef.current = window.setInterval(async () => {
      try {
        ...
        if (!t.running) {
          ...
          await load(true)
          push(
            t.state === 'done' ? '可选组件已就绪' : '任务已结束',
            t.state === 'done' ? '可在下方确认状态，重启服务后生效' : t.detail,
            t.state === 'done' ? 'success' : 'error',
          )
        }
      } catch { /* 轮询失败忽略，下一轮再试 */ }
    }, 1500)
  ...
  const run = async (fn: () => Promise<{ task_id: string }>, label: string) => {
    ...
    try {
      ...
      push(`${label}已开始`, '进度显示在下方，可继续使用其它功能', 'success')
    } catch (e) {
      push(`${label}失败`, String(e), 'error')
    } finally {
      setBusy(false)
    }
  }
  ...
  const doCancel = async () => {
    try {
      ...
      push(r.ok ? '已取消' : '取消失败', r.detail ?? '', r.ok ? 'success' : 'error')
      setTask(await getEnvTask())
    } catch (e) {
      push('取消失败', String(e), 'error')
    }
  }
  ...
  const saveEnvConfig = async () => {
    ...
    try {
      ...
      await load(true)
      push('已保存', '向量检索开关与镜像地址立即生效', 'success')
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border/60 bg-background/40 p-4">
        ...
        <div className="space-y-1 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-muted-foreground">解释器</span>
            <code className="break-all">{status?.python.executable ?? '读取中…'}</code>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline">{status?.python.in_venv ? '虚拟环境' : '系统解释器'}</Badge>
            <Badge variant="secondary">必需依赖 {(status?.requirements ?? true) ? '已就绪' : '缺失'}</Badge>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border/60 bg-background/40 p-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2 text-sm">
          <Badge variant={vectorActive ? 'default' : 'secondary'}>
            {vectorActive ? '向量 + BM25' : '仅 BM25 关键词'}
          </Badge>
          {!enabled && <span className="text-muted-foreground">已在设置中关闭向量检索</span>}
          {enabled && !depsReady && <span className="text-muted-foreground">未安装 torch / sentence-transformers</span>}
          {enabled && depsReady && !modelReady && <span className="text-muted-foreground">嵌入模型尚未下载</span>}
          {emb?.loaded && <Badge variant="outline">模型已加载</Badge>}
        </div>
        <div className="space-y-1 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-muted-foreground">可选依赖</span>
            <Badge variant={depsReady ? 'default' : 'secondary'}>{depsReady ? '已安装' : '未安装'}</Badge>
            {depsReady && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={modelReady ? 'default' : 'secondary'}>{modelReady ? '已缓存' : '未下载'}</Badge>
            <span className="text-xs text-muted-foreground">{emb?.model_name}</span>
          </div>
          {modelReady && (
            <div className="break-all text-xs text-muted-foreground">缓存路径：{emb?.model_path}</div>
          )}
          {depsReady === false && emb?.deps_error && (
            <div className="break-all text-xs text-muted-foreground">原因：{emb.deps_error}</div>
          )}
          {emb?.last_error && (
            <div className="break-all text-xs text-amber-600 dark:text-amber-300">最近一次加载失败：{emb.last_error}</div>
          )}
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Button
            ...
            onClick={() => run(() => installEmbeddingDeps(pipIndex), '安装嵌入依赖')}
          >
            ⬇ 安装嵌入依赖（torch 等）
          </Button>
          <Button
            ...
            title={depsReady ? '' : '请先安装嵌入依赖'}
            onClick={() => run(() => downloadEmbeddingModel(modelName, hfEndpoint), '下载嵌入模型')}
          >
            ⬇ 下载嵌入模型
          </Button>
          <Button size="sm" variant="ghost" disabled={busy} onClick={() => load(true)}>↻ 重新检测</Button>
          {task?.running && <Button size="sm" variant="destructive" onClick={doCancel}>取消任务</Button>}
        </div>
          ...
          <div className="mt-3 rounded-lg border border-border/60 bg-muted/30 p-3">
            <div className="mb-1 flex items-center gap-2 text-xs">
              <Badge variant={task.running ? 'secondary' : task.state === 'done' ? 'default' : 'destructive'}>
                {task.running ? '进行中' : task.state === 'done' ? '完成' : task.state === 'cancelled' ? '已取消' : '失败'}
              </Badge>
      ...
      <div className="rounded-xl border border-border/60 bg-background/40 p-4">
        <div className="mb-3 text-base font-semibold text-violet-700 dark:text-violet-200">⚙️ 向量检索配置</div>
        <div className="space-y-3">
          <label className="flex items-center justify-between gap-3 text-sm">
            <span>
              启用向量检索
              <span className="ml-2 text-xs text-muted-foreground">关闭后仅用 BM25 关键词匹配</span>
            </span>
            <Switch checked={enabled} onCheckedChange={setEnabled} />
          </label>
          <div className="space-y-1">
            <div className="text-sm">嵌入模型名</div>
            <Input value={modelName} onChange={(e) => setModelName(e.target.value)} placeholder="BAAI/bge-base-zh-v1.5" />
          </div>
          <div className="space-y-1">
            <div className="text-sm">HuggingFace 镜像（国内建议 https://hf-mirror.com）</div>
            <Input value={hfEndpoint} onChange={(e) => setHfEndpoint(e.target.value)} placeholder="留空 = 官方源" />
          </div>
          <div className="space-y-1">
            <div className="text-sm">pip 源（安装 torch 时使用，留空 = 默认源）</div>
          ...
          <p className="text-xs text-muted-foreground">
            不安装可选依赖与模型也能正常对话与会话管理，此时长期记忆只按 BM25 关键词 + 重要度 + 时效性检索。
          </p>
        </div>
      </div>
    </div>
