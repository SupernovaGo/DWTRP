...
[![LLM](https://img.shields.io/badge/LLM-DeepSeek%20API-4d6bfe)](#配置-api-key)
[![license](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![version](https://img.shields.io/badge/version-1.0-orange)](#版本历程)
...
和 SillyTavern（酒馆）这类偏向「单角色对话」工具不同，本项目更注重**多人世界的模拟**。
...
界面自带**完整的中文 / 英文支持**，可在「**设置 → 个性化 → 🌐 语言 / Language**」里随时切换。
选择会保存在浏览器里、立即生效。但注意：提示词默认中文。

---

## 配置 API Key

首次使用需要在**设置 → 连接 / 高级**里填入 DeepSeek API Key（保存在 `server/.env`），也可以手动编辑该文件（从 `server/.env.example` 复制）：
...
- **设置**：引擎参数、提示词、运行环境、个性化（含语言切换）、连接 / 高级（API Key 与原生 JSON 配置）。
